...
import { playerPokemons, presentInfoUpdate} from '../main.js';
import { calculateTeamPointsValue } from "./TeamUtils.js";

export { savedTeamsUpdate, saveTeam, deleteTeam, loadTeam }

function changeTeamToJson(jsonData) {
    playerPokemons.length = 0;
    playerPokemons.push(...JSON.parse(JSON.stringify(jsonData)));
    presentInfoUpdate();
}

function deleteTeam(teamId) {
    var savedTeams = JSON.parse(localStorage.getItem("savedTeams")) || [];
    savedTeams = savedTeams.filter(team => team.teamId !== teamId);
...
function loadTeam(teamId) {
    var savedTeams = JSON.parse(localStorage.getItem("savedTeams")) || [];
    return savedTeams.find(team => team.teamId === teamId);
}
