...
function createResults(filter = null, num = null, object = null) {
    let minPointValue = 1
    const container = document.querySelector("#SearchTab ul");
...
function filterResults(filter = null) {
    ...
    requestAnimationFrame(() => {
        allElements.forEach(el => {
            ...
            const pokemonName = aTag.getAttribute('data-name');
            console.log(aTag)
            const normalizedName = pokemonName.replace(/\s|[-.%']/g, '').toLowerCase();

            if (pokemons.some(pokemon => pokemon.name.replace(/\s|[-.%']/g, '').toLowerCase() === normalizedName)) {
...
function createMovesResults(filter = null, num = null, object = null, moveNum) {
    var results = document.querySelector('#SearchTab ul');
    results.innerHTML = '';
    ...
    li.appendChild(h3)
    document.querySelector('#SearchTab ul').appendChild(li);
